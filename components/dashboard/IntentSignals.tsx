  ...
  return (
    <div className="space-y-3">
      <ul className="space-y-2">
        {signals.map((s) => (
          <li key={`${s.id}-${s.label}`} className="flex items-start gap-2 text-sm text-[color:var(--c-muted)]">
            <span className="text-emerald-400" aria-hidden>
              ✓
            </span>
            <span>
              {s.label}
              <span className="ml-2 text-[color:var(--c-muted2)]">(+{s.points})</span>
            </span>
          </li>
        ))}
      </ul>
      <p className="font-syne text-sm font-bold text-[color:var(--c-text)]">
