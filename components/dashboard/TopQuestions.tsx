...
export default function TopQuestions({
  ...
  return (
    <div className="dash-card p-5 pl-6">
      ...
      {!items.length ? (
        <div className="rounded-[var(--radius-lg)] border border-dashed border-[color:var(--c-border)] bg-[color:var(--c-surface2)]/30 px-4 py-8 text-center">
          <p className="text-sm text-[color:var(--c-muted)] leading-relaxed">
            Todavía no hay clusters. Enviá tráfico al widget o tocá <strong className="text-[color:var(--c-text)]">Actualizar IA</strong>.
          </p>
        </div>
      ) : (
          ...
          {items.slice(0, 10).map((q, i) => (
            ...
            >
              ...
              <span className="text-[color:var(--c-muted)] leading-snug">
                {q.question}{" "}
                <span className="ml-1 font-semibold tabular-nums text-[color:var(--c-muted2)]">×{q.count}</span>
              </span>
            </li>
          ))}
        </ol>
