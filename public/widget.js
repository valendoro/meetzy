...
`))if(x.startsWith("data: "))try{let g=JSON.parse(x.slice(6));if(g.type==="text"&&g.content&&(h+=g.content,s.textContent=h,this.msgsEl.scrollTop=9999),g.type==="ui_component"&&g.component){let E=A(g.component,this.config.brandColor);this.msgsEl.appendChild(E),this.msgsEl.scrollTop=9999}}catch(g){}}this.updateChips(r.currentSection)}catch(d){s.textContent="Error de conexi\xF3n."}finally{this.isStreaming=!1,(c=this.avatarRenderer)==null||c.setTalking(!1),a==null||a.classList.remove("mz-talk")}}evaluateTriggers(){if(this.state==="chat")return;let i=this.tracker.get(),n=this.triggers.evaluate(i);if(!n)return;let e=this.triggers.getMessage(n,i);this.showMessage(e)}css(){let i=this.config.brandColor,n=this.config.widgetPosition==="bottom-left";return`
  ...
  </style>`;let s=document.createElement("div");s.className="w",e.appendChild(s);let r=document.createElement("div");r.className="s";let o=document.createElement("div");o.className="av";let c=($=i.avatarImageUrl)==null?void 0:$.trim();if(c){let p=document.createElement("img");p.className="mz-avatar-img",p.src=c,p.alt="",o.appendChild(p)}else o.textContent=i.agentName.slice(0,2).toUpperCase();r.appendChild(o);let d=document.createElement("div");d.className="an",d.textContent=i.agentName,r.appendChild(d);let l=document.createElement("div");l.className="ar",l.innerHTML=`<span class="ad"></span>${i.agentRole}`,r.appendChild(l),s.appendChild(r);let f=document.createElement("div");f.className="c";let w=document.createElement("div");w.className="m",w.id="fp-msgs",f.appendChild(w);let h=document.createElement("div");h.className="ia";let m=document.createElement("textarea");m.className="it",m.placeholder="Contame qu\xE9 necesit\xE1s\u2026",m.rows=1,m.addEventListener("input",()=>{m.style.height="auto",m.style.height=Math.min(m.scrollHeight,110)+"px"}),h.appendChild(m);let y=document.createElement("button");y.className="sb",y.innerHTML='<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M22 2L11 13M22 2L15 22L11 13M11 13L2 9" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>',h.appendChild(y),f.appendChild(h),s.appendChild(f);let x=(()=>{let p=localStorage.getItem("mz_uid");return p||(p=Math.random().toString(36).slice(2),localStorage.setItem("mz_uid",p)),p})(),g,E=!1,W=Date.now(),I=!1;function _(){if(I||!g)return;I=!0;let p=Math.round((Date.now()-W)/1e3),u=new URLSearchParams(location.search);fetch(`${k}/api/sessions/end`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({conversationId:g,visitorId:x,siteId:t,sessionDuration:p,activeTime:p,pagesVisited:[location.pathname],scrollDepth:0,device:/Mobile/i.test(navigator.userAgent)?"mobile":"desktop",browser:navigator.userAgent.includes("Chrome")?"chrome":"other",referrer:document.referrer||null,utmSource:u.get("utm_source"),utmMedium:u.get("utm_medium"),utmCampaign:u.get("utm_campaign")}),keepalive:!0}).catch(()=>{})}window.addEventListener("beforeunload",_),document.addEventListener("visibilitychange",()=>{document.visibilityState==="hidden"&&_()});function N(p,u){let C=e.getElementById("fp-msgs"),b=document.createElement("div");b.className=`mw m${p}`;let v=document.createElement("div");return v.className=`mb mb${p}`,v.textContent=u,b.appendChild(v),C.appendChild(b),C.scrollTop=9999,v}N("a",i.welcomeMessage);async function R(p){if(E)return;m.value="",m.style.height="auto",N("u",p),E=!0;let u=e.getElementById("fp-msgs"),C=document.createElement("div");C.className="mw ma";let b=document.createElement("div");b.className="mb mba",b.innerHTML='<span class="typing"><span></span><span></span><span></span></span>',C.appendChild(b),u.appendChild(C),u.scrollTop=9999;try{let v=await fetch(`${k}/api/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({siteId:t,message:p,conversationId:g,visitorId:x,plan:i.plan})});if(!v.ok||!v.body){b.textContent="Error.";return}let D=v.headers.get("X-Conversation-Id");D&&(D!==g&&(I=!1),g=D);let F=v.body.getReader(),B=new TextDecoder,H="";for(b.textContent="";;){let{done:Y,value:j}=await F.read();if(Y)break;for(let O of B.decode(j).split(`
`))if(O.startsWith("data: "))try{let P=JSON.parse(O.slice(6));P.type==="text"&&P.content&&(H+=P.content,b.textContent=H,u.scrollTop=9999)}catch(P){}}}catch(v){b.textContent="Error."}finally{E=!1}}m.addEventListener("keydown",p=>{if(p.key==="Enter"&&!p.shiftKey){p.preventDefault();let u=m.value.trim();u&&!E&&R(u)}}),y.addEventListener("click",()=>{let p=m.value.trim();p&&!E&&R(p)})}async function U(){var n,e,a;let t=(n=window.MEETZYCONFIG)==null?void 0:n.siteId;if(!t)return;window._mzWidgetInit=(e=window._mzWidgetInit)!=null?e:Date.now();let i;try{let s=await fetch(`${k}/api/sites/${t}/config`);if(!s.ok)return;let r=await s.json();if(i={...r,avatarImageUrl:(a=r.avatarImageUrl)!=null?a:null},!i.isActive)return}catch(s){return}if(i.embedMode==="fullpage"){ae(t,i);return}new z(t,i).init()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",U):U();})();
